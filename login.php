 ...
 * This page corresponds to all functionality related to user login.
 */
session_start();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $checkAcct = $pdo->prepare("SELECT user_id, username, active, suspension_remove_date FROM users WHERE username = :username");
        $checkAcct->bindParam(':username', $username);
        $checkAcct->execute();

        if ($row = $checkAcct->fetch(PDO::FETCH_ASSOC)) {
            ...
            if ($row['active'] == 0 && !is_null($row['suspension_remove_date'])) {
                ...
                if ($current_time >= $suspension_end) {
                    ...
                    $reactivate = $pdo->prepare("UPDATE users SET active = 1, suspension_remove_date = NULL WHERE username = :username");
                    $reactivate->bindParam(':username', $username);
                    $reactivate->execute();
                    ...
                    echo "Wait! Your account is suspended until " . $suspension_end_formatted . ".";
                ...
                echo "Oh no! The account is locked. Please contact the administrator.   <a href='home.html'>Return home.</a>";
                exit();
            }
        ...
        $unsuccessfulLogins = $pdo->prepare("SELECT unsuccessful_login_attempts FROM users WHERE username = :username");
        $unsuccessfulLogins->bindParam(':username', $username);
        $unsuccessfulLogins->execute();

        // Get the result
        $result = $unsuccessfulLogins->fetch(PDO::FETCH_ASSOC);
        ...
        if ($result) {
            ...
            if ($failed_attempts >= 3) {
                ...
                $userStateInfo = $pdo->prepare("UPDATE users SET active = 0, suspension_remove_date = NULL WHERE username = :username");
                $userStateInfo->bindParam(':username', $username);

                // Execute the update
                if ($userStateInfo->execute()) {
                    $rowsAffected = $userStateInfo->rowCount();
        ...
        $userLoginData = $pdo->prepare("SELECT user_id, username, password_hash, active, access_level FROM users WHERE username = :username");
        $userLoginData->bindParam(':username', $username);
        $userLoginData->execute();

        // Get user data
        $user = $userLoginData->fetch(PDO::FETCH_ASSOC);
        ...
        if ($user && password_verify($password, $user['password_hash'])) {
            ...
            if ($user['active']) {
                ...
                if ($password_result && $password_result['last_password_reset_datetime']) {
                    ...
                    if ($days_since_reset > 26) {
                        ...
                        if ($days_until_expiry <= 0) {
                            ...
                            $_SESSION['password_message'] = "<i class='fa-solid fa-triangle-exclamation'></i> Your password has expired! You must reset it immediately for security purposes.";
                            $_SESSION['password_severity'] = 'critical';
                        } else {
                            // Password expiring soon
                            $_SESSION['password_expiring'] = true;
                            $_SESSION['password_message'] = "<i class='fa-solid fa-triangle-exclamation'></i> Your password expires in " . $days_until_expiry . " day(s). Please update it soon to avoid being locked out.";
                    ...
                    $_SESSION['password_expired'] = true;
                    $_SESSION['password_message'] = "<i class='fa-solid fa-triangle-exclamation'></i> Your password has never been reset and may be using a default password. Please update it immediately for security.";
                    $_SESSION['password_severity'] = 'critical';
                ...
                try {
                    ...
                    $loginQuery = $pdo->prepare("INSERT INTO `system-access-log` (username, login_datetime, ip_address) VALUES (:username, :login_datetime, :ip_address)");

                    // Bind parameters
                    $loginQuery->bindParam(':username', $username);
                    $loginQuery->bindParam(':login_datetime', $current_datetime);
                    $loginQuery->bindParam(':ip_address', $ip_address);
                ...
                try {
                    $profileInfo = $pdo->prepare("SELECT * FROM users WHERE username = :username");
                    $profileInfo->bindParam(':username', $username);
                    $profileInfo->execute();

                    $result = $profileInfo->fetch(PDO::FETCH_ASSOC);

                    if (!$result) {
                        die("Username not found.");
                    }

                    echo "COMPLETE Field Analysis for User: " . ($username) . "<br>";
                    ...
                    foreach ($result as $field_name => $field_value) {
                        ...
                        if ($is_blank) {
                            echo "<strong style='color: red;'>" . $field_name . ":</strong> " . $status . " <i class='fa-solid fa-xmark'></i><br>";
                            $has_blank_fields = true;
                            $blank_count++;
                        } else {
                            echo $field_name . ": " . $status . " <i class='fa-solid fa-square-check'></i><br>";
                        }
                    }

                    echo "<br><strong>SUMMARY:</strong><br>";
                    if ($has_blank_fields) {
                        echo "<i class='fa-solid fa-xmark'></i> PROFILE INCOMPLETE: " . $blank_count . " field(s) missing data<br>";
                        echo "ALL fields must be populated before account can be fully activated.";
                        header("Location: profile.php");
                        exit();
                    } else {
                        echo "<i class='fa-solid fa-square-check'></i> PROFILE COMPLETE: All required fields are populated.";
